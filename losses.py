...
def ncc_loss(y_true, y_pred, eps=1e-5):
    ...
    true_c = y_true - mean_true
    pred_c = y_pred - mean_pred

    numerator = tf.reduce_mean(true_c * pred_c)

    denominator = tf.sqrt(
        tf.reduce_mean(tf.square(true_c)) *
        tf.reduce_mean(tf.square(pred_c)) + eps
    )
...
# =========================
# TOTAL LOSS (STABLE DOCTORATE VERSION)
# =========================
